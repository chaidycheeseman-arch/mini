...
(function() {
    ...
    window.openTransferModal = function() {
        ...
        var balanceRadio = document.querySelector('input[name="tf-pay-method"][value="balance"]');
    ...
    window.submitTransfer = async function() {
        ...
        var selectedPayMethod = document.querySelector('input[name="tf-pay-method"]:checked');
        var amount = parseFloat(amountVal);
        if (!amountVal || isNaN(amount) || amount <= 0) {
            document.getElementById('transfer-amount-input').focus();
            return;
        }
        if (selectedPayMethod && selectedPayMethod.value !== 'balance') {
            alert('当前转账仅支持余额支付');
            return;
        }
