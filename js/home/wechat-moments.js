...
- 线上聊天只允许发送真正会出现在聊天框里的消息内容，绝对禁止动作旁白、场景旁白、心理旁白、舞台说明；不要写 *...*、（动作）或第三人称叙述。
【递进规则】
- 本轮能 1 条说完就只发 1 条。
...
${langInstruction}`
    ...
    async function openRoleMomentsPage() {
        ...
        const coverEl = document.getElementById('role-moments-cover');
        ...
        if (coverEl) {
            const coverKey = 'role-moments-bg-img-' + activeChatContact.id;
            let savedCover = null;
            try {
                savedCover = await imgDb.images.get(coverKey);
            } catch (e) {
                savedCover = null;
            }
            if (savedCover && savedCover.src) {
                coverEl.style.backgroundImage = 'url(' + savedCover.src + ')';
