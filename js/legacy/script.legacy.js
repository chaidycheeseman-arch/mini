...
function changeImage(type) {
    ...
    // 弹窗交互：移除所有“×”关闭按钮，统一通过点击空白区域关闭
    function initPopupCloseBehavior() {
        const popupRootSelector = '#preset-manager, #worldbook-editor, [id$="-modal"], [id$="-overlay"]';
        function removePopupCloseXButtons() {
            document.querySelectorAll(`${popupRootSelector} [onclick]`).forEach(el => {
                const txt = (el.textContent || '').trim();
                const handler = (el.getAttribute('onclick') || '').replace(/\s+/g, '');
                if (txt === '×' && /^close[A-Za-z0-9_]*\(\)$/.test(handler)) {
                    el.remove();
                }
            });
        }

        removePopupCloseXButtons();

        // 动态插入的弹窗（如运行时创建）也要持续移除“×”
        const mo = new MutationObserver(() => removePopupCloseXButtons());
        mo.observe(document.body, { childList: true, subtree: true });

        const backdropCloseMap = {
            'shura-confrontation-overlay': () => {
                if (typeof closeShuraConfrontation === 'function') closeShuraConfrontation();
                else {
                    const el = document.getElementById('shura-confrontation-overlay');
                    if (el) el.style.display = 'none';
                }
            },
            'tf-pay-pwd-overlay': () => {
                if (typeof closeTfPayPwd === 'function') closeTfPayPwd();
                else {
                    const el = document.getElementById('tf-pay-pwd-overlay');
                    if (el) el.style.display = 'none';
                }
            },
            'rp-pay-pwd-overlay': () => {
                if (typeof closeRpPayPwd === 'function') closeRpPayPwd();
                else {
                    const el = document.getElementById('rp-pay-pwd-overlay');
                    if (el) el.style.display = 'none';
                }
            }
        };

        Object.keys(backdropCloseMap).forEach(id => {
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    const ST_MM_CACHE = 'miffy_minimax_voice_cache_v1';
    const MM_CACHE_LIMIT = 60;
    const MM_LANG_TEST_TEXT = {
        ...
        yue: '喂，你好呀，MiniMax 语音连接正常。'
    };
    const CONTACT_LANG_TO_MM_LANG = {
        '中': 'zh',
        '中文': 'zh',
        '英': 'en',
        '英文': 'en',
    ...
    function normalizeMiniMaxLanguage(lang) {
        ...
        if (val === 'en' || val === 'ja' || val === 'ko' || val === 'yue') return val;
        return 'zh';
    }
    ...
    function getMiniMaxLanguageByContact(contact) {
        if (!contact || !contact.roleLanguage) return '';
        const key = String(contact.roleLanguage).trim();
        return CONTACT_LANG_TO_MM_LANG[key] || '';
    }

    function applyContactLanguageToMiniMaxConfig(config, contact) {
        const contactLanguage = getMiniMaxLanguageByContact(contact);
        if (!contactLanguage) return normalizeMiniMaxConfig(config || {});
        return normalizeMiniMaxConfig(Object.assign({}, config || {}, {
    ...
    function buildMiniMaxCacheKey(text, config) {
        const normalized = `${config.region}|${config.groupId}|${config.voiceId}|${config.language}|${config.speed}|${text}`;
        return miniMaxHash(normalized);
    }

    async function getMiniMaxCacheList() {
        const cache = await localforage.getItem(ST_MM_CACHE);
        return Array.isArray(cache) ? cache : [];
    }

    async function saveMiniMaxCacheList(cacheList) {
        const list = Array.isArray(cacheList) ? cacheList : [];
        list.sort((a, b) => (b.lastUsedAt || 0) - (a.lastUsedAt || 0));
        await localforage.setItem(ST_MM_CACHE, list.slice(0, MM_CACHE_LIMIT));
    }

    async function getMiniMaxCachedVoice(cacheKey) {
        const list = await getMiniMaxCacheList();
        const idx = list.findIndex(item => item && item.key === cacheKey && item.audioHex);
        if (idx === -1) return null;
        const hit = list[idx];
        hit.lastUsedAt = Date.now();
        list.splice(idx, 1);
        list.unshift(hit);
        await saveMiniMaxCacheList(list);
        return hit;
    }

    async function upsertMiniMaxCachedVoice(cacheKey, config, text, audioHex) {
        const list = await getMiniMaxCacheList();
        const idx = list.findIndex(item => item && item.key === cacheKey);
        const now = Date.now();
        const item = {
            key: cacheKey,
        ...
        if (idx !== -1) {
            item.createdAt = list[idx].createdAt || now;
            list.splice(idx, 1);
        }
        list.unshift(item);
        await saveMiniMaxCacheList(list);
    ...
    function stopMiniMaxAudioPlayback() {
        if (mmCurrentAudio) {
    ...
    async function synthesizeMiniMaxVoice(text, config) {
        const normalizedConfig = normalizeMiniMaxConfig(config || {});
        const cleanText = (text || '').trim();
        if (!cleanText) throw new Error('语音内容为空');
        if (!normalizedConfig.apiKey) throw new Error('请先填写 MiniMax API 密钥');
        if (!normalizedConfig.groupId) throw new Error('请先填写 Group ID');
        if (!normalizedConfig.voiceId) throw new Error('请先填写语音 ID');
        const cacheKey = buildMiniMaxCacheKey(cleanText, normalizedConfig);
        const cached = await getMiniMaxCachedVoice(cacheKey);
        if (cached) return { audioHex: cached.audioHex, fromCache: true };
        const audioHex = await requestMiniMaxVoiceHex(cleanText, normalizedConfig);
        await upsertMiniMaxCachedVoice(cacheKey, normalizedConfig, cleanText, audioHex);
        return { audioHex: audioHex, fromCache: false };
    }
    ...
    async function testMiniMaxConnection() {
        ...
        try {
            ...
            const result = await synthesizeMiniMaxVoice(testText, config);
            showMiniMaxTestTip(testText, false);
            await playMiniMaxHexAudio(result.audioHex, null);
        } catch (e) {
    ...
    async function loadSettingsToUI() {
        ...
        if (langEl) {
            const contactLanguage = getMiniMaxLanguageByContact(activeChatContact);
            langEl.value = contactLanguage || mmConfig.language || 'zh';
        }
    ...
    function applyPresetManagerTitle() {
        ...
        if (titleEl) titleEl.textContent = presetManagerMode === 'voice' ? '语音预设管理' : '预设管理';
    }
...
// ====== 小说 Tab 切换 ======
    ...
    const SPECIAL_MESSAGE_TYPE_ALIASES = Object.freeze({
        ...
        call_invite: 'call',
        video_call_response: 'video_call',
        ai_image: 'image',
        polaroid: 'camera',
    ...
    function normalizeSpecialMessageType(rawType) {
        const type = String(rawType || '').trim();
        return SPECIAL_MESSAGE_TYPE_ALIASES[type] || type;
    }

    function normalizeSpecialMessageObject(rawObj) {
        if (!rawObj || typeof rawObj !== 'object' || Array.isArray(rawObj)) return rawObj;
        const obj = Object.assign({}, rawObj);
        obj.type = normalizeSpecialMessageType(obj.type);

        if (obj.type === 'text') {
            obj.content = (obj.content === null || obj.content === undefined) ? '' : String(obj.content);
        } else if (obj.type === 'voice_message') {
            const voiceBody = obj.content || obj.transcript || obj.translation || '';
            obj.content = String(voiceBody || '');
        } else if (obj.type === 'camera') {
            const cameraBody = obj.content || obj.text || obj.description || obj.desc || '';
            obj.content = String(cameraBody || '');
        } else if (obj.type === 'location') {
            obj.address = obj.address || obj.name || obj.content || '未知位置';
            obj.distance = obj.distance || '';
        } else if (obj.type === 'red_packet') {
            const amountNum = parseFloat(String(obj.amount !== undefined ? obj.amount : obj.totalAmount).replace(/[^\d.]/g, ''));
            obj.amount = (!isNaN(amountNum) && amountNum >= 0) ? amountNum.toFixed(2) : '0.00';
            obj.desc = obj.desc || obj.greeting || obj.content || '恭喜发财，大吉大利';
            obj.status = obj.status || 'unclaimed';
        } else if (obj.type === 'transfer') {
            const amountNum = parseFloat(String(obj.amount).replace(/[^\d.]/g, ''));
            obj.amount = (!isNaN(amountNum) && amountNum >= 0) ? amountNum.toFixed(2) : '0.00';
            obj.desc = obj.desc || obj.note || obj.content || '转账';
            obj.status = obj.status || 'pending';
        } else if (obj.type === 'takeaway') {
            let fallbackItem = obj.item || obj.food || obj.name || obj.restaurant;
            if (!fallbackItem && Array.isArray(obj.items) && obj.items.length > 0) {
                const firstItem = obj.items[0] || {};
                fallbackItem = firstItem.name || firstItem.item || firstItem.restaurant;
            }
            obj.item = fallbackItem || '外卖';
            obj.desc = obj.desc || obj.note || obj.eta || '给你点了外卖';
        } else if (obj.type === 'gift') {
            let fallbackGift = obj.item || obj.name;
            if (!fallbackGift && Array.isArray(obj.items) && obj.items.length > 0) {
                const firstGift = obj.items[0] || {};
                fallbackGift = firstGift.name || firstGift.item;
            }
            obj.item = fallbackGift || '礼物';
            obj.desc = obj.desc || obj.message || '送你的礼物';
        } else if (obj.type === 'payment_request') {
            const fallbackItem = {
                name: obj.product || obj.productInfo || '代付账单',
                desc: obj.desc || '',
                price: parseFloat(obj.price || obj.amount || obj.totalAmount) || 0
            };
            obj.items = Array.isArray(obj.items) ? obj.items : [fallbackItem];
            obj.note = obj.note || obj.message || '帮我代付一下';
        } else if (obj.type === 'payment_receipt') {
            obj.items = Array.isArray(obj.items) ? obj.items : [];
            obj.total = parseFloat(obj.total || obj.amount || 0) || 0;
            obj.message = obj.message || obj.note || '';
        }

        return obj;
    }

    function _stripMarkdownJsonFence(rawText) {
    ...
    function _parseInstructionCandidate(text) {
        if (!text || typeof text !== 'string') return null;
        try {
            const parsed = JSON.parse(text);
            if (Array.isArray(parsed)) {
                return parsed.filter(function(item) { return item && typeof item === 'object' && !Array.isArray(item); });
            }
    ...
    function _extractFirstBalancedJson(text, openChar, closeChar) {
        if (!text) return '';
    ...
    function _extractBalancedJsonObjects(text) {
        const objects = [];
    ...
    function parseAiReplyInstructions(replyText) {
        const cleanText = _stripMarkdownJsonFence(replyText);
        if (!cleanText) return [];

        const candidates = [];
        candidates.push(cleanText);
        const fullArray = _extractFirstBalancedJson(cleanText, '[', ']');
        if (fullArray) candidates.push(fullArray);
        const fullObject = _extractFirstBalancedJson(cleanText, '{', '}');
        if (fullObject) candidates.push(fullObject);

        for (let i = 0; i < candidates.length; i++) {
            const parsed = _parseInstructionCandidate(candidates[i]);
            if (parsed && parsed.length > 0) return parsed;
        }

        // 兜底：提取所有完整对象（支持嵌套结构），避免正则截断嵌套 JSON。
        const objectChunks = _extractBalancedJsonObjects(cleanText);
        if (objectChunks.length > 0) {
            const parsedObjects = [];
            objectChunks.forEach(function(chunk) {
                const parsed = _parseInstructionCandidate(chunk);
                if (parsed && parsed.length > 0) parsedObjects.push.apply(parsedObjects, parsed);
            });
            if (parsedObjects.length > 0) return parsedObjects;
        }

        const textLines = cleanText.split('\n').map(function(line) { return line.trim(); }).filter(Boolean);
        if (textLines.length === 0) return [{ type: 'text', content: cleanText }];
    ...
    function extractMsgPureText(content) {
        if (!content) return '';
        try {
            const parsed = normalizeSpecialMessageObject(JSON.parse(content));
            if (parsed.type === 'text' && parsed.translation) return parsed.translation || parsed.content || '';
            if (parsed.type === 'voice_message') return '[语音] ' + (parsed.translation || parsed.content || '');
            ...
            if (parsed.type === 'takeaway') return `[外卖] ${parsed.item || parsed.name || ''}`.trim();
            if (parsed.type === 'gift') return `[礼物] ${parsed.item || parsed.name || ''}`.trim();
            if (parsed.type === 'call') return `[语音通话] ${parsed.status || ''}`.trim();
            if (parsed.type === 'video_call') return `[视频通话] ${parsed.status || ''}`.trim();
            if (parsed.type === 'payment_request') return `[代付账单]`;
            if (parsed.type === 'payment_receipt') return `[支付小票]`;
    ...
    function generateMsgHtml(msg, myAvatar, roleAvatar) {
        ...
        // 外语双语气泡兼容：避免把已存的原文/翻译结构当纯文本转义，导致气泡严重掉格式
        function _buildBilingualBubbleHtml(raw) {
            if (!raw || typeof raw !== 'string') return '';
            const m = raw.match(/<div class="msg-original-text">([\s\S]*?)<\/div>\s*<div class="msg-translate-divider"><\/div>\s*<div class="msg-translated-text">([\s\S]*?)<\/div>/i);
            if (!m) return '';
            const toPlain = (html) => {
        ...
        let voiceText = '';
        let voiceTranslatedText = '';
        let voiceSeconds = 0;
        let isStructuredCardMsg = false;
        try {
            // 尝试解析 JSON 格式
            const parsed = normalizeSpecialMessageObject(JSON.parse(msg.content));
            if (parsed && parsed.type === 'camera') {
                ...
                isVoiceMsg = true;
                voiceText = parsed.content || parsed.translation || '';
                voiceTranslatedText = (parsed.translation && parsed.translation !== voiceText) ? parsed.translation : '';
                ...
                `;
            } else if (parsed && ['takeaway', 'gift', 'call', 'video_call', 'payment_request', 'payment_receipt'].includes(parsed.type)) {
                statusHtml = '';
                isStructuredCardMsg = true;
                const reqItems = Array.isArray(parsed.items) ? parsed.items : [];
                const reqTotal = reqItems.reduce(function(sum, it) {
                    const unitPrice = parseFloat(it && it.price) || 0;
                    const qty = parseInt((it && (it.qty || it.count)) || 1, 10) || 1;
                    return sum + (unitPrice * qty);
                }, 0);
                const reqFirst = reqItems[0] || {};
                const reqTitle = reqFirst.name || reqFirst.item || reqFirst.product || '代付账单';
                const reqTitleWithCount = reqItems.length > 1 ? (reqTitle + ' 等' + reqItems.length + '件') : reqTitle;
                const receiptTotal = (parsed.total !== undefined && parsed.total !== null && !isNaN(parseFloat(parsed.total)))
                    ? parseFloat(parsed.total)
                    : reqTotal;
                const cardMap = {
                    takeaway: {
                        title: parsed.item || parsed.name || '给你点了外卖',
                        subtitle: parsed.desc || '热乎的，记得趁热吃',
                        badge: '外卖已下单',
                        footer: 'Mini 外卖卡片',
                        icon: '<svg viewBox="0 0 24 24" fill="none" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M4 7h13l-1.2 6.2a2 2 0 0 1-2 1.6H8.2a2 2 0 0 1-2-1.6L4 7Z"></path><path d="M17 9h2.5a2.5 2.5 0 0 1 0 5H17"></path><circle cx="9" cy="18.5" r="1.5"></circle><circle cx="16" cy="18.5" r="1.5"></circle></svg>'
                    },
                    gift: {
                        title: parsed.item || parsed.name || '送你的礼物',
                        subtitle: parsed.desc || '拆开看看喜不喜欢',
                        badge: '礼物送达',
                        footer: 'Mini 礼物卡片',
                        icon: '<svg viewBox="0 0 24 24" fill="none" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="8" width="18" height="12" rx="2"></rect><path d="M12 8v12"></path><path d="M3 12h18"></path><path d="M7.5 8a2.5 2.5 0 1 1 0-5c2 0 4.5 2.4 4.5 5"></path><path d="M16.5 8a2.5 2.5 0 1 0 0-5c-2 0-4.5 2.4-4.5 5"></path></svg>'
                    },
                    call: {
                        title: parsed.status || '发起语音通话',
                        subtitle: parsed.desc || '想直接听听你的声音',
                        badge: '语音邀请',
                        footer: 'Mini 通话卡片',
                        icon: '<svg viewBox="0 0 24 24" fill="none" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.9v3a2 2 0 0 1-2.2 2 19.8 19.8 0 0 1-8.6-3.1 19.4 19.4 0 0 1-6-6A19.8 19.8 0 0 1 2.1 4.2 2 2 0 0 1 4.1 2h3a2 2 0 0 1 2 1.7l.4 2.6a2 2 0 0 1-.6 1.8l-1.5 1.5a16 16 0 0 0 6 6l1.5-1.5a2 2 0 0 1 1.8-.6l2.6.4A2 2 0 0 1 22 16.9Z"></path></svg>'
                    },
                    video_call: {
                        title: parsed.status || '发起视频通话',
                        subtitle: parsed.desc || '想看看你现在的样子',
                        badge: '视频邀请',
                        footer: 'Mini 视频卡片',
                        icon: '<svg viewBox="0 0 24 24" fill="none" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="6" width="13" height="12" rx="2"></rect><path d="m16 10 5-3v10l-5-3z"></path></svg>'
                    },
                    payment_request: {
                        title: reqTitleWithCount,
                        subtitle: parsed.note || '帮我代付一下',
                        badge: '待你代付',
                        footer: '合计 ¥' + reqTotal.toFixed(2),
                        icon: '<svg viewBox="0 0 24 24" fill="none" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="16" rx="2"></rect><line x1="7" y1="9" x2="17" y2="9"></line><line x1="7" y1="13" x2="14" y2="13"></line><line x1="7" y1="17" x2="12" y2="17"></line></svg>'
                    },
                    payment_receipt: {
                        title: reqTitleWithCount || '支付小票',
                        subtitle: parsed.message || '支付已完成',
                        badge: '支付完成',
                        footer: '实付 ¥' + receiptTotal.toFixed(2),
                        icon: '<svg viewBox="0 0 24 24" fill="none" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M6 3h12v18l-3-2-3 2-3-2-3 2V3z"></path><line x1="8" y1="8" x2="16" y2="8"></line><line x1="8" y1="12" x2="16" y2="12"></line><line x1="8" y1="16" x2="13" y2="16"></line></svg>'
                    }
                };
                const card = cardMap[parsed.type];
                msgBodyHtml = `
                    <div class="chat-special-card special-${parsed.type}">
                        <div class="chat-special-card-top">
                            <div class="chat-special-card-icon">${card.icon}</div>
                            <div class="chat-special-card-text">
                                <div class="chat-special-card-title">${_safeTextHtml(card.title)}</div>
                                <div class="chat-special-card-subtitle">${_safeTextHtml(card.subtitle)}</div>
                            </div>
                        </div>
                        <div class="chat-special-card-footer">
                            <span class="chat-special-card-badge">${card.badge}</span>
        ...
        if (isCameraMsg) {
            ...
            statusHtml = ''; // 隐藏双✓，保持纯净
            const voiceTranslationHtml = voiceTranslatedText
                ? `<div class="voice-translate-hint">${_safeTextHtml(voiceTranslatedText)}</div>`
                : '';
            msgBodyHtml = `
                <div class="voice-msg-container" onclick="toggleVoiceText(this)">
                    ...
                    <div class="voice-expand-area">
                        ...
                        ${voiceTranslationHtml}
                    </div>
                </div>
            `;
        }
        const isBareBubble = isCameraMsg || isImageMsg || isEmoticonMsg || isLocationMsg || isStructuredCardMsg || (() => {
            try {
                const p = normalizeSpecialMessageObject(JSON.parse(msg.content));
                return p.type === 'red_packet' || p.type === 'transfer';
            } catch(e) {
                return false;
            }
        })();
        ...
        return `
            <div class="chat-msg-row ${msgClass}" data-id="${msg.id}" data-sender="${msg.sender}">
                ...
                <div class="msg-bubble-wrapper" style="position:relative;">
                    <div class="chat-msg-content msg-content-touch" style="${isBareBubble ? 'background:transparent; box-shadow:none; padding:0;' : ''}">
                        ${quoteHtml}
                        ${msgBodyHtml}
                        ${statusHtml}
                    </div>
                    <div class="chat-timestamp" style="${isBareBubble ? 'display:none;' : ''}">${msg.timeStr}</div>
                    ${_blockedBadgeHtml}
                </div>
            </div>
        `;
    ...
    async function enterChatWindow(contactId) {
        ...
        const mmLangEl = document.getElementById('mm-lang');
        const mmContactLang = getMiniMaxLanguageByContact(contact);
        if (mmLangEl && mmContactLang) mmLangEl.value = mmContactLang;
    ...
    async function openRoleProfile() {
        ...
        if (coverSection && !coverSection._rpClickBound) {
            ...
            coverSection.addEventListener('click', function(e) {
                ...
                e.stopPropagation();
                // 设置当前目标为封面背景
                openImageMenuAtEvent(e, 'rp-cover-bg-img');
            });
        }
    }
    ...
    async function triggerRoleReply() {
        ...
        try {
            ...
            const baseSpecialProbConfig = await getContactSpecialMessageProbabilities(lockedContact);
            const specialRuntimeContext = _buildSpecialMessageRuntimeContext(rawMessages);
            const specialProbConfig = _buildRuntimeSpecialMessageProbabilities(baseSpecialProbConfig, specialRuntimeContext);
            const exclusiveSpecialType = _rollExclusiveSpecialMessageType(specialProbConfig);
            const triggerCamera = exclusiveSpecialType === 'camera';
            const triggerLocation = exclusiveSpecialType === 'location';
            ...
            const triggerRedPacket = exclusiveSpecialType === 'red_packet';
            const triggerTransfer = exclusiveSpecialType === 'transfer';
            const latestUserMessage = rawMessages.slice().reverse().find(function(m) { return m.sender === 'me'; });
            const latestUserText = latestUserMessage ? (extractMsgPureText(latestUserMessage.content) || '') : '';
            const forceTakeawayByIntent = /(外卖|点餐|奶茶|夜宵|咖啡|饿了|吃的)/i.test(latestUserText);
            const forceGiftByIntent = /(礼物|惊喜|送我|送你|买给我)/i.test(latestUserText);
            const forceCallByIntent = /(打电话|语音通话|语音电话|电话聊|通话)/i.test(latestUserText);
            const forceVideoCallByIntent = /(视频通话|视频电话|开视频|视频聊|视频吧)/i.test(latestUserText);
            const forcePaymentRequestByIntent = /(代付|帮我付|付款请求|你付款|帮忙付款)/i.test(latestUserText);
            const forcePaymentReceiptByIntent = /(支付小票|付款小票|买单小票|付款成功小票|代付小票)/i.test(latestUserText);
            if (forceTakeawayByIntent) triggerTakeaway = true;
            if (forceGiftByIntent) triggerGift = true;
            if (forceCallByIntent) triggerCall = true;
            if (forceVideoCallByIntent) triggerVideoCall = true;
            let triggerVoice = Math.random() < (_normalizeProbabilityValue(specialProbConfig.voice_message, DEFAULT_SPECIAL_MESSAGE_PROBABILITIES.voice_message) / 100);
            let triggerEmoticon = (allEmoticons.length > 0) && (Math.random() < (_normalizeProbabilityValue(specialProbConfig.emoticon, DEFAULT_SPECIAL_MESSAGE_PROBABILITIES.emoticon) / 100));
            // 读取时间感知开关
            ...
            const triggerReply = Math.random() < (_normalizeProbabilityValue(specialProbConfig.reply, DEFAULT_SPECIAL_MESSAGE_PROBABILITIES.reply) / 100);
            let triggerRecall = Math.random() < (_normalizeProbabilityValue(specialProbConfig.recall, DEFAULT_SPECIAL_MESSAGE_PROBABILITIES.recall) / 100);
            const triggerTimeAwareReply = timeAwareOn && (Math.random() < (_normalizeProbabilityValue(specialProbConfig.time_aware, DEFAULT_SPECIAL_MESSAGE_PROBABILITIES.time_aware) / 100));
            var hardSpecialCandidates = [];
            if (triggerVoice) hardSpecialCandidates.push('voice_message');
            if (triggerEmoticon) hardSpecialCandidates.push('emoticon');
            if (triggerRecall) hardSpecialCandidates.push('recall_msg');
            var hardSpecialLimit = exclusiveSpecialType ? 1 : 2;
            var keptHardSpecials = _pickSpecialHardTriggers(hardSpecialCandidates, hardSpecialLimit, specialRuntimeContext);
            triggerVoice = keptHardSpecials.includes('voice_message');
            triggerEmoticon = keptHardSpecials.includes('emoticon');
            triggerRecall = keptHardSpecials.includes('recall_msg');
            if (triggerRedPacket || triggerTransfer) {
                triggerVoice = false;
                triggerEmoticon = false;
                triggerRecall = false;
            }
            ...
            if (triggerReply) {
                ...
                specialFeatures.push(`${featureIndex++}. 当你想针对某句话进行回复时，使用 "type": "reply"，并在 "target_text" 摘录用户最近一句里的原文片段，回复内容要紧贴该片段语义。`);
            }
            ...
            if (triggerVoice) {
                ...
                specialFeatures.push(`${featureIndex++}. 【强制】本次回复中必须包含至少一条语音，使用 "type": "voice_message"。语音文案必须是即时口语（8~35字），像真人一口气说出来，不要书面腔。`);
            }
            if (triggerEmoticon) {
            ...
            const pendingRp = rawMessages.slice().reverse().find(m => {
                ...
                try {
                    const p = normalizeSpecialMessageObject(JSON.parse(m.content));
                    return p.type === 'red_packet' && p.status === 'unclaimed';
            ...
            const pendingTf = rawMessages.slice().reverse().find(m => {
                ...
                try {
                    const p = normalizeSpecialMessageObject(JSON.parse(m.content));
                    return p.type === 'transfer' && p.status === 'pending';
                } catch(e) { return false; }
            });
            if (pendingRp) {
                allowedTypes.push("open_red_packet");
                typeInstructions.push(`{"type": "open_red_packet", "content": "哇谢谢你的红包！"}`);
                specialFeatures.push(`${featureIndex++}. 用户发送了红包还未被领取，你可以选择使用 "type": "open_red_packet" 来领取它（content为你的反应文字），系统会自动更新红包状态并显示提示。`);
            }
            if (pendingTf) {
                allowedTypes.push("accept_transaction");
                typeInstructions.push(`{"type": "accept_transaction", "action": "received", "content": "收到转账啦谢谢"}`);
                specialFeatures.push(`${featureIndex++}. 用户发送了转账还未处理，你可以选择使用 "type": "accept_transaction" 来接收（action为"received"）或退回（action为"refunded"），content为你的反应文字，系统会自动更新状态并显示提示。`);
            }
            if (triggerTakeaway) {
                allowedTypes.push("takeout_delivery");
                typeInstructions.push(`{"type": "takeout_delivery", "items": [{"name": "外卖物品", "price": 35.5, "qty": 1}], "note": "给你点了外卖", "eta": "预计30分钟达"}`);
                specialFeatures.push(`${featureIndex++}. 【强制】本次回复中必须包含点外卖动作，使用 "type": "takeout_delivery"。`);
            }
            if (triggerGift) {
                allowedTypes.push("send_gift");
                typeInstructions.push(`{"type": "send_gift", "items": [{"name": "礼物名称", "price": 99, "qty": 1}], "message": "送你的礼物"}`);
                specialFeatures.push(`${featureIndex++}. 【强制】本次回复中必须包含送礼物动作，使用 "type": "send_gift"。`);
            }
            if (triggerCall) {
                allowedTypes.push("call_invite");
                typeInstructions.push(`{"type": "call_invite", "status": "发起语音通话"}`);
                specialFeatures.push(`${featureIndex++}. 【强制】本次回复中必须包含发起电话动作，使用 "type": "call_invite"。`);
            }
            if (triggerVideoCall) {
                allowedTypes.push("video_call_response");
                typeInstructions.push(`{"type": "video_call_response", "status": "发起视频通话"}`);
                specialFeatures.push(`${featureIndex++}. 【强制】本次回复中必须包含发起视频动作，使用 "type": "video_call_response"。`);
            }
            // ====== 新增：时间感知回复特殊提示（按配置+上下文概率，开关开启时触发） ======
            ...
            var typeUsageRules = [];
            typeUsageRules.push('总则：以下“何时使用”默认按双向理解，既包括用户对你提出需求，也包括你主动向用户发起需求。');
            if (allowedTypes.includes('text')) {
                typeUsageRules.push('text：默认类型。用于承接上下文、表达情绪、推进对话；不需要动作或系统能力时只发 text。');
            }
            if (allowedTypes.includes('reply')) {
                typeUsageRules.push('reply：当你要精准回应用户某一句原话时使用，target_text 必须摘录对方刚说过的片段，内容要围绕该片段。');
            }
            if (allowedTypes.includes('recall_msg')) {
                typeUsageRules.push('recall_msg：仅在“说错话/冲动泄露心声/害羞后悔”这类心理瞬间使用，不可频繁连续触发。');
            }
            if (allowedTypes.includes('voice_message')) {
                typeUsageRules.push('voice_message：当你想更亲密、强调情绪、表达安抚或被用户要求“语音/说话/打电话”时使用；语音文案必须短促口语化。');
            }
            if (allowedTypes.includes('emoticon')) {
                typeUsageRules.push('emoticon：当文字不足以表达情绪（撒娇、无语、开心、委屈、起哄）时使用；不要每轮都发。');
            }
            if (allowedTypes.includes('camera')) {
                typeUsageRules.push('camera：当你在“分享当下场景/证明你正在做某事/回应用户想看你周围环境”时使用。');
            }
            if (allowedTypes.includes('location')) {
                typeUsageRules.push('location：当用户问你或你想知道用户在哪、要见面、查岗、确认行程位置时使用。');
            }
            if (allowedTypes.includes('takeout_delivery')) {
                typeUsageRules.push('takeout_delivery：当你想照顾用户（怕他饿、加班、熬夜、生病）或顺势表达关心时使用。');
            }
            if (allowedTypes.includes('send_gift')) {
                typeUsageRules.push('send_gift：当出现纪念日、道歉补偿、庆祝、制造惊喜等节点时使用。');
            }
            if (allowedTypes.includes('red_packet')) {
                typeUsageRules.push('red_packet：用于节日祝福、感谢、哄人开心、活跃气氛的小额心意；不用于严肃的大额支付。');
            }
            if (allowedTypes.includes('transaction')) {
                typeUsageRules.push('transaction：用于明确金额的报销、代付、补偿或承诺兑现；金额与备注必须具体。');
            }
            if (allowedTypes.includes('open_red_packet')) {
                typeUsageRules.push('open_red_packet：仅当用户有未领取红包时使用，表达“领取后”的真实反应。');
            }
            if (allowedTypes.includes('accept_transaction')) {
                typeUsageRules.push('accept_transaction：仅当用户有待处理转账时使用；根据语境选择 received（收下）或 refunded（退回）。');
            }
            if (allowedTypes.includes('call_invite')) {
                typeUsageRules.push('call_invite：当你强烈想实时沟通、想念对方、或有紧急/重要内容不适合打字时使用。');
            }
            if (allowedTypes.includes('video_call_response')) {
                typeUsageRules.push('video_call_response：当你希望更强临场感（见面感、展示状态、确认对方情况）时使用。');
            }
            var typeUsageInstruction = typeUsageRules.length
                ? typeUsageRules.map(function(rule, idx) { return (idx + 1) + '. ' + rule; }).join('\n')
                : '1. 本次仅允许 text，正常自然聊天即可。';
            // 如果是外语，为typeInstructions增加translation字段
            if (roleLang !== '中') {
                typeInstructions = typeInstructions.map(instr => {
                    return instr.replace(/\}\s*$/, ', "translation": "中文翻译"}');
                });
            }
...
【角色优先级】先准确回应眼前这段对话，再决定是否使用特殊消息。特殊消息只是表达形式，绝不能为了凑功能而突然跑题、胡言乱语、破坏人设或忽略上下文。
...
【类型使用时机（必须遵守）】
${typeUsageInstruction}
【特殊消息演技】所有特殊消息都必须先接住当前对话语境再触发：语音要像自然开口，定位/图片/外卖/礼物要有合理动机，不要机械凑功能。
...
绝对不要输出任何Markdown代码块标记（如\`\`\`json），直接输出纯JSON数组！`
            ...
            if (lockedContact.worldbooks && lockedContact.worldbooks.length > 0) {
                ...
                // 构建最近消息的纯文本，用于关键词匹配
                const recentPureText = recentMessages.map(m => buildMessagePromptText(m, lockedContact)).join(' ');
                wbs.forEach(wb => {
            ...
            recentMessages.forEach(msg => {
                ...
                if (isImage) {
                    ...
                    const imagePromptText = buildMessagePromptText(msg, lockedContact) || (msg.sender === 'me' ? '[用户发送了一张图片，请仔细识别图片内容并作出回应]' : '[角色发送了一张图片]');
                    messages.push({
                        role: msg.sender === 'me' ? 'user' : 'assistant',
                        content: [
                            {
                    ...
                    let promptText = buildMessagePromptText(msg, lockedContact);
        ...
        const endpoint = `${cleanApiUrl}/v1/chat/completions`;
            ...
            let replyArr = parseAiReplyInstructions(replyText);
            if (!Array.isArray(replyArr) || replyArr.length === 0) {
                replyArr = [{ type: 'text', content: replyText }];
            }
            replyArr = replyArr.map(function(item) {
                if (!item || typeof item !== 'object' || Array.isArray(item)) {
                    return { type: 'text', content: String(item || '') };
                }
                return normalizeSpecialMessageObject(item);
            ...
            if (triggerVoice) {
                ...
                if (!hasVoiceMessage) {
                    ...
                    const fallbackText = _normalizeVoiceMessageDraft(
                        fallbackTextObj && fallbackTextObj.content
                    ...
                    if (replyArr.length <= 1) replyArr.push(fallbackVoice);
                    else replyArr.splice(1, 0, fallbackVoice);
                }
                var firstVoiceIndex = replyArr.findIndex(function(item) {
                    return item && item.type === 'voice_message' && (item.content || item.transcript);
                });
                if (firstVoiceIndex > 1) {
                    var voiceMsg = replyArr.splice(firstVoiceIndex, 1)[0];
                    replyArr.splice(1, 0, voiceMsg);
                }
            }

            if (triggerEmoticon && allEmoticons.length > 0) {
                const hasValidEmoticon = replyArr.some(function(item) {
                    if (!item || item.type !== 'emoticon') return false;
                    return allEmoticons.some(function(emo) { return emo.url === item.content; });
                });
                if (!hasValidEmoticon) {
                    const randomEmoticon = allEmoticons[Math.floor(Math.random() * allEmoticons.length)];
                    if (randomEmoticon && randomEmoticon.url) {
                        replyArr.push({
                            type: 'emoticon',
                            desc: randomEmoticon.desc || '表情',
                            content: randomEmoticon.url
                        });
                    }
                }
            }

            // 逐条渲染回复：首条更快，语音优先提速，提升体感即时性
            for (let i = 0; i < replyArr.length; i++) {
                const msgObj = normalizeSpecialMessageObject(replyArr[i] || {});
                // 红包、转账、处理红包/转账类型可能没有 content 字段，需要单独放行
                const noContentTypes = ['red_packet', 'transfer', 'handle_red_packet', 'handle_transfer', 'takeaway', 'gift', 'call', 'video_call', 'payment_request', 'payment_receipt'];
                ...
                const renderDelay = i === 0
                    ? 900
                    : (msgObj.type === 'voice_message' ? 700 : 1300);
                ...
                if (msgObj.type === 'camera') {
                    ...
                    finalContent = JSON.stringify({
                        type: 'voice_message',
                        content: msgObj.content || msgObj.translation || '',
                    ...
                    continue;
                } else if (['takeaway', 'gift', 'call', 'video_call', 'payment_request', 'payment_receipt'].includes(msgObj.type)) {
                    finalContent = JSON.stringify(normalizeSpecialMessageObject(msgObj));
    ...
    async function viewRecalledMsg(msgId) {
        ...
        if (msg) {
            ...
            try {
                const parsed = normalizeSpecialMessageObject(JSON.parse(msg.content));
                if (parsed.type === 'voice_message') displayHtml = '[语音] ' + (parsed.translation || parsed.content || '');
                else if (parsed.type === 'camera') displayHtml = '[相片] ' + (parsed.content || '');
                else if (parsed.type === 'image') displayHtml = '[图片]';
                else if (parsed.type === 'transfer') displayHtml = '[转账] ' + (parsed.note || parsed.desc || ('¥' + (parsed.amount || '0.00')));
                else if (parsed.type === 'takeaway') displayHtml = '[外卖] ' + (parsed.item || parsed.name || '');
                else if (parsed.type === 'gift') displayHtml = '[礼物] ' + (parsed.item || parsed.name || '');
                else if (parsed.type === 'call') displayHtml = '[语音通话] ' + (parsed.status || '');
                else if (parsed.type === 'video_call') displayHtml = '[视频通话] ' + (parsed.status || '');
                else if (parsed.type === 'payment_request') displayHtml = '[代付账单]';
                else if (parsed.type === 'payment_receipt') displayHtml = '[支付小票]';
    ...
    // 控制语音展开与波纹动画，并接入 MiniMax 语音播放（带缓存）
    async function toggleVoiceText(element) {
        ...
        if (expandArea.style.display === 'flex') {
            ...
            mmVoiceRequestSeq++;
            if (mmCurrentVoiceElement === element) {
                stopMiniMaxAudioPlayback();
            }
            return;
        }
        ...
        const storedConfig = await getMiniMaxStoredConfig();
        const config = applyContactLanguageToMiniMaxConfig(storedConfig, activeChatContact);
        if (!config.apiKey || !config.groupId || !config.voiceId) return;

        const requestId = ++mmVoiceRequestSeq;
        try {
            const result = await synthesizeMiniMaxVoice(text, config);
            if (requestId !== mmVoiceRequestSeq || !element.classList.contains('expanded')) return;
            await playMiniMaxHexAudio(result.audioHex, element);
        } catch (e) {
        ...
        try {
            ...
            if (selectedKeys.includes('settings') || selectedKeys.includes('theme') || selectedKeys.includes('contacts') || selectedKeys.includes('voice')) {
                ...
                for (let key of lfKeys) {
                    ...
                    if (selectedKeys.includes('voice') && key.startsWith('miffy_minimax_')) shouldExport = true;
                    if (shouldExport) {
...
(function() {
    ...
    const LEGACY_DEFAULT_SPECIAL_MESSAGE_PROBABILITIES = Object.freeze({
        camera: 0.1,
        location: 0.1,
        takeaway: 0.1,
        gift: 0.1,
    ...
    const SPECIAL_MESSAGE_PROBABILITY_VERSION = 2;
    ...
    function _isSameSpecialProbabilityConfig(a, b) {
        if (!a || !b) return false;
        var keys = Object.keys(DEFAULT_SPECIAL_MESSAGE_PROBABILITIES);
        return keys.every(function(key) {
            var av = _normalizeProbabilityValue(a[key], 0);
            var bv = _normalizeProbabilityValue(b[key], 0);
            return Math.abs(av - bv) < 0.001;
        });
    }

    function _getMessageTypeForSpecialRuntime(rawContent) {
        try {
            var parsed = JSON.parse(rawContent);
            return parsed && parsed.type ? String(parsed.type) : 'text';
        } catch(e) {
            return 'text';
        }
    }

    function _isSpecialRuntimeType(type) {
        return !!type && type !== 'text' && type !== 'image';
    }

    function _buildSpecialMessageRuntimeContext(messages) {
        var list = Array.isArray(messages) ? messages.slice(-40) : [];
        var roleVoiceGap = 0;
        var roleSpecialGap = 0;
        var roleVoiceFound = false;
        var roleSpecialFound = false;
        var userTurnCounter = 0;
        var userSentVoiceRecently = false;
        var userRecentlyUsedEmoticon = false;
        var lastUserText = '';

        for (var i = list.length - 1; i >= 0; i--) {
            var msg = list[i];
            var type = _getMessageTypeForSpecialRuntime(msg && msg.content);
            if (msg && msg.sender === 'role') {
                if (!roleVoiceFound) {
                ...
                if (!roleSpecialFound) {
                    if (_isSpecialRuntimeType(type)) {
                        roleSpecialFound = true;
                    } else {
                        roleSpecialGap++;
                    }
                }
            } else if (msg && msg.sender === 'me') {
                userTurnCounter++;
                if (!lastUserText) {
                    lastUserText = (extractMsgPureText(msg.content) || '').trim();
                }
                if (userTurnCounter <= 4 && type === 'voice_message') userSentVoiceRecently = true;
                if (userTurnCounter <= 4 && type === 'emoticon') userRecentlyUsedEmoticon = true;
            }
        }

        if (!roleVoiceFound) roleVoiceGap = 12;
        if (!roleSpecialFound) roleSpecialGap = 12;

        var userRequestedVoice = /(语音|说句话|说话|声音|打电话|通话|电话|voice|call)/i.test(lastUserText);
        var lastUserTextHasEmotion = /(想你|好想|难过|委屈|生气|哭|抱抱|亲亲|爱你|喜欢|想见你|焦虑|吃醋|嫉妒|紧张|开心|崩溃)/i.test(lastUserText);

        return {
            roleVoiceGap: roleVoiceGap,
            roleSpecialGap: roleSpecialGap,
            userSentVoiceRecently: userSentVoiceRecently,
            userRecentlyUsedEmoticon: userRecentlyUsedEmoticon,
            userRequestedVoice: userRequestedVoice,
            lastUserText: lastUserText,
            lastUserTextLength: lastUserText.length,
            lastUserTextHasEmotion: lastUserTextHasEmotion
        };
    }

    function _boostSpecialProbability(config, key, delta) {
        config[key] = _normalizeProbabilityValue((config[key] || 0) + delta, config[key] || 0);
    }

    function _buildRuntimeSpecialMessageProbabilities(baseConfig, runtimeContext) {
        var config = _normalizeSpecialMessageProbabilities(baseConfig);
        var ctx = runtimeContext || {};

        if (ctx.roleSpecialGap >= 6) {
            EXCLUSIVE_SPECIAL_MESSAGE_KEYS.forEach(function(key) {
                _boostSpecialProbability(config, key, 1.2);
            });
            _boostSpecialProbability(config, 'reply', 8);
            _boostSpecialProbability(config, 'emoticon', 6);
        }
        if (ctx.roleSpecialGap >= 10) {
            EXCLUSIVE_SPECIAL_MESSAGE_KEYS.forEach(function(key) {
                _boostSpecialProbability(config, key, 1.5);
            });
        }
        if (ctx.roleVoiceGap >= 4) _boostSpecialProbability(config, 'voice_message', 15);
        if (ctx.roleVoiceGap >= 8) _boostSpecialProbability(config, 'voice_message', 12);
        if (ctx.userSentVoiceRecently) _boostSpecialProbability(config, 'voice_message', 18);
        if (ctx.userRequestedVoice) _boostSpecialProbability(config, 'voice_message', 28);
        if (ctx.userRecentlyUsedEmoticon) _boostSpecialProbability(config, 'emoticon', 12);
        if (ctx.lastUserTextLength >= 18) _boostSpecialProbability(config, 'reply', 8);
        if (ctx.lastUserTextLength >= 36) _boostSpecialProbability(config, 'reply', 6);
        if (ctx.lastUserTextHasEmotion) {
            _boostSpecialProbability(config, 'recall', 8);
            _boostSpecialProbability(config, 'time_aware', 4);
        }

        return config;
    }

    function _pickSpecialHardTriggers(candidates, maxCount, runtimeContext) {
        ...
        if (ctx.userRequestedVoice || ctx.userSentVoiceRecently || ctx.roleVoiceGap >= 5) {
            weights.voice_message += 6;
        }
        if (ctx.userRecentlyUsedEmoticon) weights.emoticon += 2;
        if (ctx.lastUserTextHasEmotion) weights.recall_msg += 2;
    ...
    function _normalizeVoiceMessageDraft(rawText) {
        var text = (rawText === null || rawText === undefined) ? '' : String(rawText);
        text = text.replace(/\s+/g, ' ').trim().replace(/^[“"'`]+|[”"'`]+$/g, '');
        if (!text) return '嗯，我在听。';
        if (text.length > 34) text = text.slice(0, 34).replace(/[，。；、,.;!?！？\s]+$/g, '');
        if (text.length < 4) text += '，我在呢。';
        return text || '嗯，我在听。';
    }
    ...
    async function getContactSpecialMessageProbabilities(contact) {
        ...
        try {
            var key = CD_KEY_PREFIX + target.id + '_special_message_probabilities';
            var versionKey = CD_KEY_PREFIX + target.id + '_special_message_probabilities_version';
            var saved = await localforage.getItem(key);
            if (!saved) return _getDefaultSpecialMessageProbabilities();
            ...
            if (savedVersion < SPECIAL_MESSAGE_PROBABILITY_VERSION && _isSameSpecialProbabilityConfig(normalized, LEGACY_DEFAULT_SPECIAL_MESSAGE_PROBABILITIES)) {
                normalized = _getDefaultSpecialMessageProbabilities();
    ...
    function _describeTransferStatus(status) {
        if (status === 'received' || status === 'accepted') return '已收款';
        if (status === 'refunded' || status === 'declined') return '已退回';
        if (status === 'claimed') return '已领取';
        return '待处理';
    }

    function buildMessagePromptText(msg, contact) {
        if (!msg) return '';
        var senderName = msg.sender === 'me'
            ? ((document.getElementById('text-wechat-me-name') && document.getElementById('text-wechat-me-name').textContent) || '我')
            : ((contact && contact.roleName) || '角色');
        if (msg.isRecalled) return senderName + '：[撤回了一条消息]';

        var contentText = '';
        try {
            var parsed = normalizeSpecialMessageObject(JSON.parse(msg.content));
            if (parsed.type === 'text') {
                contentText = parsed.translation
                    ? ('[文本] 原文：' + (parsed.content || '') + '；翻译：' + parsed.translation)
                    : (parsed.content || '');
            } else if (parsed.type === 'voice_message') {
                var voiceBody = parsed.translation || parsed.content || '';
                contentText = parsed.translation && parsed.content
                    ? ('[语音] 原文：' + parsed.content + '；翻译：' + parsed.translation)
                    : ('[语音] ' + voiceBody);
            } else if (parsed.type === 'camera') {
                contentText = '[相片] ' + (parsed.content || '');
            } else if (parsed.type === 'image') {
                contentText = '[图片]';
            } else if (parsed.type === 'emoticon') {
                contentText = '[表情包] ' + (parsed.desc || '');
            } else if (parsed.type === 'location') {
                contentText = '[定位] ' + (parsed.address || parsed.name || '未知位置') + (parsed.distance ? ('，' + parsed.distance) : '');
            } else if (parsed.type === 'red_packet') {
                contentText = '[红包] 金额¥' + (parsed.amount || '0.00') + '，祝福：' + (parsed.greeting || parsed.desc || '恭喜发财，大吉大利') + '，状态：' + _describeTransferStatus(parsed.status || 'unclaimed');
            } else if (parsed.type === 'transfer') {
                contentText = '[转账] 金额¥' + (parsed.amount || '0.00') + '，备注：' + (parsed.note || parsed.desc || '转账') + '，状态：' + _describeTransferStatus(parsed.status || 'pending');
            } else if (parsed.type === 'takeaway') {
                contentText = '[外卖] ' + (parsed.item || parsed.name || '外卖') + (parsed.desc ? ('，说明：' + parsed.desc) : '');
            } else if (parsed.type === 'gift') {
                contentText = '[礼物] ' + (parsed.item || parsed.name || '礼物') + (parsed.desc ? ('，说明：' + parsed.desc) : '');
            } else if (parsed.type === 'call') {
                contentText = '[语音通话] ' + (parsed.status || '发起了语音通话');
            } else if (parsed.type === 'video_call') {
                contentText = '[视频通话] ' + (parsed.status || '发起了视频通话');
            } else if (parsed.type === 'payment_request') {
                contentText = '[代付账单] ' + (parsed.note || '请帮我代付');
            } else if (parsed.type === 'payment_receipt') {
                contentText = '[支付小票] 合计¥' + ((parsed.total !== undefined && parsed.total !== null) ? parsed.total : '0.00');
            } else if (parsed.content) {
                contentText = String(parsed.content);
            }
        } catch(e) {}

        if (!contentText) contentText = extractMsgPureText(msg.content);

        if (msg.quoteText) {
            try {
                var qData = JSON.parse(msg.quoteText);
                var qName = qData.name || '对方';
                var qContent = qData.content || '';
                contentText = '[回复 ' + qName + '：' + qContent + '] ' + contentText;
            } catch(e) {}
        }
    ...
    window.cdSaveSpecialProbabilities = async function() {
        ...
        await localforage.setItem(CD_KEY_PREFIX + activeChatContact.id + '_special_message_probabilities_version', SPECIAL_MESSAGE_PROBABILITY_VERSION);
    };
